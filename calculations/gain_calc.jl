...
# ╔═╡ 9429e9b4-120f-417b-9e16-5b6d44b22709
using WGLMakie

# ╔═╡ 2ca79315-398e-406e-aa07-ee5d1a09145e
# ╠═╡ disabled = true
#=╠═╡
using PlutoUI
  ╠═╡ =#

# ╔═╡ e1d6789a-c3b0-11f0-22cf-655dce833427
R_GV = 75 .+ collect(0:128).*(10e3/128)

# ╔═╡ 14a21878-eb7f-45d1-8c00-5f5a6705164b
begin
	R_F = [120e3, 220e3, 330e3, 470e3, 630e3];
	# @bind R_GP PlutoUI.Slider(0e3:10e3:100e3);
	# @bind R_GS PlutoUI.Slider(0e3:1e3:10e3);
	R_GP = [1e6, 2e6, 3e6, 4e6, 5e6];
	R_GS = [680, 1.0e3, 1.2e3, 2.2e3, 3.3e3];
end

# ╔═╡ 2028dbeb-7965-4c16-b8ce-217cd86ebee2
function gain(RF, RGP, RGS, RGV)
	G = 1 .+ RF ./ (RGS .+ RGV .* RGP ./ (RGV .+ RGP))
end

# ╔═╡ a30770b5-c903-4f6c-bb9c-9dce365da47e
gain(R_F[3], R_GP[5], R_GS[2], R_GV)

# ╔═╡ 77c67802-cf63-49ce-bb03-76ccc974119e
gain(100e3, 1e9, 330, R_GV)

# ╔═╡ d7bf1c42-2ee5-491d-9ed4-5dc490a47dad
[R_F[3], R_GP[5], R_GS[2]]

# ╔═╡ 9e131def-795a-4656-b335-1ae7e5e9a5f2
begin
	fig1 = Figure()
	ax1 = Axis(fig1[1,1], yautolimitmargin=(0.1,0.1), xautolimitmargin = (0.1, 0.1))
	for i in 1:5
		lines!(ax1, collect(0:128), gain(R_F[i], R_GP[5], R_GS[1], R_GV), alpha=(0.3+0.1*i), color=:blue)
	end
	fig1
end

# ╔═╡ f051b017-6278-4f4a-889a-369582daab74
begin
	fig2 = Figure()
	ax2 = Axis(fig2[1,1], yautolimitmargin=(0.1,0.1), xautolimitmargin = (0.1, 0.1))
	for i in 1:5
		lines!(ax2, collect(0:128), gain(R_F[3], R_GP[i], R_GS[3], R_GV), alpha=(0.3+0.1*i), color=:red)
	end
	fig2
end

# ╔═╡ d2b853d0-cf6e-474a-a461-f5792b4a20f1
begin
	fig3 = Figure()
	ax3 = Axis(fig3[1,1], yautolimitmargin=(0.1,0.1), xautolimitmargin = (0.1, 0.1))
	for i in 1:5
		lines!(ax3, collect(0:128), gain(R_F[4], R_GP[5], R_GS[i], R_GV), alpha=(0.3+0.1*i), color=:green)
	end
	fig3
end

# ╔═╡ 17614d22-36c4-43dc-bdee-390d69cdb740
begin
	fig4 = Figure()
	ax4 = Axis(fig4[1,1], yautolimitmargin=(0.1,0.1), xautolimitmargin = (0.1, 0.1))
	for i in 1:5
		lines!(ax4, collect(0:128), gain(R_GV, 1e6, 1, 50), alpha=(0.3+0.1*i), color=:blue)
	end
	fig4
end
...
manifest_format = "2.0"
project_hash = "a0dabce00828993e4ba0272bf725105fcb148bb5"
...
git-tree-sha1 = "548dd16e479145c8ba8f8545b854ccc898a43ff6"
...
git-tree-sha1 = "c5309cf02b9a7fbf51a3f56658117fbb2288f34d"
