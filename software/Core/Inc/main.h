/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2022 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32g0xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define ENCODER_BUTTON_Pin GPIO_PIN_0
#define ENCODER_BUTTON_GPIO_Port GPIOA
#define INA_OUT_Pin GPIO_PIN_1
#define INA_OUT_GPIO_Port GPIOA
#define DISPLAY_ENABLE_Pin GPIO_PIN_2
#define DISPLAY_ENABLE_GPIO_Port GPIOA
#define BRIDGE_ENABLE_Pin GPIO_PIN_3
#define BRIDGE_ENABLE_GPIO_Port GPIOA
#define DPOT_HVC_A0_Pin GPIO_PIN_4
#define DPOT_HVC_A0_GPIO_Port GPIOA
#define GPIO_PA5_Pin GPIO_PIN_5
#define GPIO_PA5_GPIO_Port GPIOA
#define ENCODER_A_Pin GPIO_PIN_6
#define ENCODER_A_GPIO_Port GPIOA
#define ENCODER_B_Pin GPIO_PIN_7
#define ENCODER_B_GPIO_Port GPIOA
#define GPIO_PB0_Pin GPIO_PIN_0
#define GPIO_PB0_GPIO_Port GPIOB
#define GPIO_PB1_Pin GPIO_PIN_1
#define GPIO_PB1_GPIO_Port GPIOB
#define GPIO_PB2_Pin GPIO_PIN_2
#define GPIO_PB2_GPIO_Port GPIOB
#define GPIO_PA8_Pin GPIO_PIN_8
#define GPIO_PA8_GPIO_Port GPIOA
#define INA_ENABLE_Pin GPIO_PIN_15
#define INA_ENABLE_GPIO_Port GPIOA
#define GPIO_PB3_Pin GPIO_PIN_3
#define GPIO_PB3_GPIO_Port GPIOB
#define GPIO_PB4_Pin GPIO_PIN_4
#define GPIO_PB4_GPIO_Port GPIOB
#define GPIO_PB5_Pin GPIO_PIN_5
#define GPIO_PB5_GPIO_Port GPIOB
/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
